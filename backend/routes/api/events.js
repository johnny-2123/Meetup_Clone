...
const {
  ResultWithContext,
} = require("express-validator/src/chain/context-runner-impl.js");
...
router.get("/", async (req, res) => {
  ...
  if (Number.isNaN(page)) {
    res.status(400);
    return res.json({
      errors: [{ message: "page number must be a valid integer" }],
    });
  }
  ...
  if (Number.isNaN(size)) {
    res.status(400);
    return res.json({
      errors: [{ message: "size number must be a valid integer" }],
    });
  }
  if (size < 0) {
    size = 10;
  ...
  if (type && typeof type !== "string") {
    res.status(400);
    return res.json({
      errors: [{ message: "type must be a string" }],
    });
  }
  ...
  if (startDate && typeof startDate !== "string") {
    res.status(400);
    return res.json({
      errors: [{ message: "startDate must be a string" }],
    });
  }
  if (name) {
    where.name = {
  ...
  const events2 = await Event.findAll({
    include: [
      ...
      { model: Group, attributes: ["id", "name", "city", "state", "private"] },
    ],
    attributes: [
      "id",
      "groupId",
